    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div>
                ...
                @endif
                <div class="files">
                    @foreach($folders->_embedded->items as $item)
                        @if($item->type == 'dir')
                            <div class="file-item js-open-dir" data-link="{{route('folder', ['folder' => $item->path])}}">
                                <img src="img/folder-144.png" alt="">
                                <p>{{$item->name}}</p>
                            </div>
                        @else
                            <div data-file="{{$item->file}}" class="file-item js-get-file js-get-preview">
                                <img src="{{$item->preview ?? 'img/file-144.png'}}" data-path="{{$item->path}}" alt="">
                                <p>{{$item->name}}</p>
                            </div>
                        @endif
                    @endforeach
